...
rm -rf "local-provider/public/"

concurrently --kill-others \
    "cd frontend && npm run reload" \
    --names  "Reload" \
    --prefix-colors "#CC0099"

concurrently --kill-others \
    "cd frontend && npm run provide-local" \
    --names "Build-to-Provide" \
    --prefix-colors "#50F862"

concurrently --kill-others \
    "cd mock-api && npm start" \
    "cd frontend && npm run start" \
     "cd local-provider && npm start" \
    --names "MockApi,Frontend","Provide-Local" \
    --prefix-colors "#81688d,#848377","#ffac00"
